...
ADD gradle-${GRADLE_VERSION}-bin.zip /opt/gradle.zip
WORKDIR /opt
RUN yum install -y yum-plugin-ovl
RUN yum install -y unzip java-1.8.0-openjdk && yum clean all && unzip gradle.zip && ln -s gradle-${GRADLE_VERSION} gradle && rm gradle.zip
